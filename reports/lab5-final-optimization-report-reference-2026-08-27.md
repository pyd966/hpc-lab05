...
从优化维度看，这些工作并不是一条线上的重复调参：

| 优化维度 | 代表工作 | 直接目的 |
| --- | --- | --- |
| 质量与表示 | GPTQ group64、量化缓存 | 建立不再变化的 W4A16 精度和 checkpoint 基线 |
| 容量管理 | Offloading、Ring、Paged、共享 KV pool | 消除 OOM，并为 batch/常驻提供空间 |
| 算子与布局 | fused W4A16、FlashAttention、blocked INT4 | 减少临时张量、无效访存和 kernel 时间 |
| 请求调度 | Continuous Batching、稳定 slot、生命周期 credit | 提高有效 batch，安全复用 KV |
| 数据搬运 | 合并 payload、双 buffer、RoPE 去重、MLP 常驻 | 减少 copy 次数和 PCIe 总字节数 |

## 4. 质量与可复现实验基础

### 4.1 GPTQ W4A16：先把模型压进显存并守住质量
...
### 6.2 双 GPU buffer、合并 payload 与只读释放
...
但 TTFT 反而增加 15.8%，说明初版 tile 对 decode 更有效。完整 10 请求 batch 1 为
`400.511 s`，相比双 buffer 的 `425.020 s` 有改善，但仍远离 36 秒。
...
### 7.2 自写 Triton Flash/Paged Attention
...
## 8. 主线三：跨请求调度与消除 CPU-GPU 同步

### 8.1 Continuous Batching 与稳定 KV slot
...
### 8.3 Prefill 只计算最后有效 token 的 logits
...
## 9. 主线四：根据新热点重写 W4A16 数据布局与调度

Continuous 之后的 small4 profile 为：W4A16 `22.833 s/59.91%`，pinned H2D
`14.280 s/37.47%`，FlashAttention 只有 `57.222 ms/0.15%`。因此下一步应先修 W4，
而不是继续优化 attention。

### 9.1 N-contiguous blocked INT4 布局
...
### 9.2 Group64 scale 广播与 prefill/decode 专用调度
...
W4 已显著下降，H2D 首次成为第一热点。53 次完整模型遍历仍各搬约 5.799 GB，总量约
307.37 GB。这个 profile 直接决定最后一轮不再只调 kernel，而是先减少必须搬的字节。

## 10. 主线五：把节省的 KV 显存换成更少的 H2D

### 10.1 真正缩小共享 Paged KV 物理池
...
### 10.3 44/48 层 MLP 常驻与 attention-only Offloading
...
### 10.4 RoPE cache 去重与 cuBLAS 提前初始化

**触发证据。** 代码检查发现 48 层 payload 各有一份 FP32 RoPE cache，合计约 224 MiB，
不仅占 host/pinned 空间，还在每次遍历重复 H2D。另一个失败作业表明，若最大 prefill 后
才首次调用 LM head，cuBLAS handle/workspace 的惰性创建可能在持久显存已经占满后报
`CUBLAS_STATUS_ALLOC_FAILED`。

**目的与作用对象。** RoPE 去重针对不必要的重复存储和 H2D；cuBLAS 提前初始化针对
显存分配顺序和稳定性。

**实现。** 40 个 sliding layer 共享一份 GPU RoPE cache，8 个 global layer 共享另一份；
offloader tensor filter 排除这两份常驻 buffer。engine 在大块 MLP/KV 分配前，用 16x16
BF16 矩阵乘初始化 cuBLAS handle/workspace，且发生在正式计时前。

**结果与证据。** 最终三次正式运行和一次 full10 profile 均无 allocator warning。
RoPE、共享 pool、常驻策略和初始化顺序属于同一最终组合轮，没有逐项 full10 A/B，报告
只能使用代码/字节统计和稳定性现象分别证明其作用，不能把最终加速按比例拆给四个子项。
